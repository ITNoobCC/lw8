...
describe('Cashbox', () => {
  it ('При передаче отрицательного amount, вернет ошибку ввода', () => {
    ...
    assert.deepEqual(cashboxTest.addPayment({ amount: -20, info: 'Оплата штрафа' }),'Error enter!');
  });
  it ('Передача положительного amount и информации об оплате, вернет "Информация: amount"', () => {
    const cashboxTest = new cashbox();
    assert.deepEqual(cashboxTest.addPayment({ amount: 10, info: 'Оплата ЖКХ' }),'Оплата ЖКХ' + ': ' + 10);
  });
  it ('Передача пустого значения amount + информацией об оплате "","info", вернет ошибку ввода!', () => {
    const cashboxTest = new cashbox();
    assert.deepEqual(cashboxTest.addPayment({ amount: '', info: 'Оплата ЖКХ' }),'Error enter!');
  });
  it ('Передача пустого значени amount и info, вернет ошибку ввода!', () => {
    const cashboxTest = new cashbox();
    assert.deepEqual(cashboxTest.addPayment({ amount: '', info: '' }),'Error enter!');
  });
  it ('Передача пустого значения об информации оплате, вернет ошибку ввода!', () => {
    const cashboxTest = new cashbox();
    assert.deepEqual(cashboxTest.addPayment({ amount: '10', info: '' }),'Error enter!');
  });
  it ('Возврат клиенту имеющуюся сумму в кассе с информацией о возврате, вернет "Возврат: amount"', () => {
    const cashboxTest = new cashbox();
    assert.deepEqual(cashboxTest.refundPayment({ amount: 10, info: 'Возврат клиенту' }),'Возврат клиенту' + ': ' + 10);
  });
  it ('Передача пустого значения amount + информацией о возврате "","возврат", вернет ошибку ввода!', () => {
    const cashboxTest = new cashbox();
    assert.deepEqual(cashboxTest.refundPayment({ amount: '', info: 'Возврат клиенту' }),'Error enter!');
  });
  it ('Передача пустого значени возврата amount и info, вернет ошибку ввода!', () => {
    const cashboxTest = new cashbox();
    assert.deepEqual(cashboxTest.refundPayment({ amount: '', info: '' }),'Error enter!');
  });
  it ('Передача пустого значения о возврате оплаты, вернет ошибку ввода!', () => {
    const cashboxTest = new cashbox();
    assert.deepEqual(cashboxTest.refundPayment({ amount: '10', info: '' }),'Error enter!');
  });
});
...
describe('replaceString', () => {
  ...
  it('Передача в функцию с заменой подстроки в нескольких местах: "Hello world", "word", "world Sem", вернет "Hello world Sem"', () => {
    assert.deepEqual(replaceString('Hello world', 'world', 'world Sem'), "Hello world Sem");
  });
});
...
describe('Player', () => {
  it('Вызов функции Dispaly(), вернет "One - Status: stop"', () => {
    var playerTest = new Player.Player([new Player.Song('One', 1), new Player.Song('Two', 2), new Player.Song('Three', 3)]);
  ...
  it('Вызов функции Pause(), вернет "Noize - Status: pause"', () => {
    var playerTest = new Player.Player([new Player.Song('Noize', 1), new Player.Song('Two', 2), new Player.Song('Three', 3)]);
    playerTest.pause();
    assert.deepEqual(playerTest.display(), 'Track: Noize - Status: pause');
  });
  
  it('Вызов функции Stop(), вернет "Good Job - Status: stop"', () => {
    var playerTest = new Player.Player([new Player.Song('Good Job', 1), new Player.Song('Two', 2), new Player.Song('Three', 3)]);
    playerTest.stop();
